...
Machine translation (MT) is one of the oldest problems in artificial intelligence \cite{hutchins2004georgetown}. Over the decades, the task of automatically translating text between languages has moved from rule-based systems to statistical methods and, more recently, to neural approaches \cite{stahlberg2020neural}.

The Europarl corpus \cite{koehn2005europarl}, which contains proceedings of the European Parliament and is distributed as part of the OPUS collection \cite{tiedemann2020opus}, is a widely used benchmark in MT research. It offers large-scale, sentence-aligned parallel text across many European language pairs, which makes it a good fit for training and evaluating translation systems.
...
MT has gone through several major shifts over the years. Early statistical MT (SMT) systems like phrase-based models \cite{koehn2003statistical} relied on word and phrase alignment statistics from parallel corpora. These were eventually replaced by neural machine translation (NMT) approaches, which learn the translation task end-to-end with encoder-decoder architectures \cite{sutskever2014sequence, bahdanau2015neural}.

The Transformer architecture \cite{vaswani2017attention} was a major breakthrough, allowing for highly parallel training and better handling of long-range dependencies through self-attention. Pretrained models like MarianMT \cite{junczys2018marian} and mBART \cite{liu2020multilingual} have since made high-quality NMT much more accessible through transfer learning and fine-tuning.

\subsection{Cross-Lingual Representations}

Cross-lingual sentence embeddings \cite{reimers2020making} project sentences from different languages into a shared vector space. Models like paraphrase-multilingual-MiniLM \cite{wang2020minilm} make it possible to compare meaning across languages without explicit translation. Although these representations are mainly intended for tasks like semantic similarity and retrieval, they can also be used for translation by returning the closest target-language sentence from a pool of candidates.

\subsection{The Europarl Corpus}

The Europarl corpus \cite{koehn2005europarl}, hosted within the OPUS collection \cite{tiedemann2020opus}, is one of the most commonly used resources in MT research. It contains proceedings of the European Parliament, aligned at the sentence level across up to 21 European languages. Its formal register and high alignment quality make it well suited for benchmarking translation systems. In our project, all three models (both baselines and our fine-tuned model) are trained and evaluated on the same English-French Europarl subset.
...
Our second baseline uses pretrained multilingual sentence embeddings from the \texttt{paraphrase-multilingual-MiniLM-L12-v2} model \cite{wang2020minilm, reimers2020making}. During training, we embed all unique French sentences from the training set (up to 50,000 candidates) into a shared multilingual vector space.
...
For our main model, we start from the pretrained Helsinki-NLP/opus-mt-en-fr checkpoint \cite{junczys2018marian}, which is a MarianMT encoder-decoder Transformer already trained on a large collection of English-French parallel data. We then fine-tune this model on the same Europarl training data used by both baselines, selecting hyperparameters through grid search.
...
We evaluate all three models using sentence-level BLEU score \cite{papineni2002bleu} with smoothing (method 1 from \texttt{nltk}). BLEU measures n-gram overlap between predicted and reference translations, and is a standard automatic metric for translation quality. We also report exact match rate.
...
\caption{N-gram BLEU breakdown across all three models. Our model (blue) keeps high scores at all n-gram levels, while Baseline~1 drops sharply beyond unigrams and Baseline~2 remains low throughout.}
\label{fig:ngram_bleu}
\end{figure}

\subsection{Qualitative Analysis}

We also look at specific translation examples to get a sense of the qualitative differences between models.
...
\medskip
Baseline~1 outputs a jumble of individually correct French words with no grammatical structure. Baseline~2 picks a topically related but completely different sentence from the pool. Our model produces a fluent and accurate translation that closely matches the reference. Figure~\ref{fig:attention} shows the cross-attention weights for a sample translation, giving some insight into how the model aligns source and target tokens.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/attention_heatmap.png}
\caption{Cross-attention heatmap for our Seq2Seq model. The weights show how each generated French token attends to specific English source tokens, reflecting learned word alignment.}
\label{fig:attention}
\end{figure}

\section{Discussion}

The results clearly show the expected hierarchy: word-level lookup performs worst, semantic retrieval is slightly better, and the fine-tuned Seq2Seq model is far ahead of both. A few observations are worth highlighting.

\textbf{Baseline~1 limitations.} The word-for-word model cannot capture word order, morphological agreement, or context. Its decent BLEU-1 shows it can get individual words right, but the lack of any multi-word coherence makes its output essentially unusable.

\textbf{Baseline~2 limitations.} The embedding retrieval model can only return sentences from its candidate pool, so it can never produce a genuinely new translation. If the pool does not contain a close match, the output will be off. Its lower BLEU-1 compared to Baseline~1 also shows that sentence-level semantic similarity does not guarantee lexical overlap, which is what BLEU actually measures.

\textbf{Benefits of fine-tuning.} Our model benefits from the pretrained MarianMT weights, which already encode a lot of translation knowledge from large-scale training. Fine-tuning on Europarl then adapts it to this specific domain. The key advantage over the baselines is that our model actually \emph{learns to generate} translations rather than relying on lookup or retrieval. We also found that label smoothing hurt performance in this setting, possibly because the Europarl domain is fairly homogeneous and the pretrained model already provides enough regularization on its own.

\textbf{Sentence length effects.} As shown in Figure~\ref{fig:bleu_by_length}, all models perform worse on longer sentences. Our model is the most robust, holding up relatively well even for sentences of 40--50+ words, while Baseline~1 degrades the fastest.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{figures/bleu_by_length.png}
\caption{Average BLEU score by input sentence length. All models degrade on longer sentences, but our Seq2Seq model is the most robust.}
