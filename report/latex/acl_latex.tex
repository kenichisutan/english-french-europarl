...
\author{
Maxime Hayakawa Ivanovic \quad Juan Kenichi Sutan \quad Amine Ouatmani \\
Universit\'{e} Paris-Saclay \\
\texttt{\{maxime.hayakawa-ivanovic, juan-kenichi.sutan, amine.ouatmani\}} \\
...
As shown in Figure~\ref{fig:bleu_by_length}, all models perform worse on longer sentences. Our model is the most robust, while Baseline~2 degrades the fastest. This is consistent with its failure mode: the finite candidate pool becomes less likely to contain a close match for longer inputs, making retrieval increasingly unreliable as sentence length grows.
